...
def createNetwork():
    ...
    return s, action, readout # s and past_a are all placeholders 

'''
trainNetwork - the training process
'''
def trainNetwork(s, action, readout):
    '''
...
def plot_data():
    ...
    # plot stepList
    plt.plot(successList)
    plt.xlabel('episode({})'.format(SUCCESS_RATE_TEST_STEP))
    plt.ylabel('steps')
    plt.savefig('success_rate', dpi=1200)
    # plt.show()
    return
...
'''
def StartTraining():
    s, action, readout = createNetwork()
    trainNetwork(s, action, readout)
