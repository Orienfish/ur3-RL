...
TEST_ROUND = 50 # how many episodes in the test
# This file is the dqn reinforcement learning.
...
def trainNetwork():
	...
	if BASED_VERSION: # check whether there's a based version
        ...
        for p in IMAGE_PATH:
            train_env.append(env.FocusEnv(p)) # init an environment
		...
		past_info_t = action_t
			...
			print(past_info_t1) 
					...
					training : False}
                    	...
                    	print("EPISODE", i, "/ TIMESTEP", t, "/ GRP", train_env[l].train_data_dir, "/ STEP", step, "/ STATE", state, \
...
def testNetwork():
    ...
    for t in IMAGE_PATH:
    	...
    	train_env.append(env.FocusEnv(t))
    action_space = train_env[0].actions
    for t in TEST_PATH:
        test_env.append(env.FocusEnv(t))
		...
		step = 0
        	...
        	while True:
	    		...
	    		# print(readout_t)
            		...
            		action_t1 = np.append(action_new, action_t[:PAST_FRAME-1], axis=0)
            		# print test info
            		print("TEST EPISODE", test, "/ TIMESTEP", step, "/ GRP", train_env[l].train_data_dir, \
