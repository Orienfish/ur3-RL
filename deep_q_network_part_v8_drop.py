...
VERSION = "star_with_drop_v2"
BASED_VERSION = "v11"
LOG_DIR = PATH + "/trainlog/" + VERSION
TRAIN_DIR = PATH + "/training/train_" + VERSION
BASED_DIR = PATH + "/training/train_" + BASED_VERSION
# if directory does not exist, new it
if not os.path.isdir(TRAIN_DIR):
    os.makedirs(TRAIN_DIR)
# the following files are all in training directories
READ_NETWORK_DIR = BASED_DIR + "/saved_networks_part_" + BASED_VERSION
SAVE_NETWORK_DIR = TRAIN_DIR + "/saved_networks_part_" + VERSION
...
FILE_SUCCESS = TRAIN_DIR + "/success_rate_" + VERSION + ".txt"
...
h_relu_fc1 = tf.nn.relu(h_fc1) # [None, 256]
h_drop_fc1 = tf.nn.dropout(h_relu_fc1, keep_prob=0.5)
...
h_relu_fc2 = tf.nn.relu(h_fc2) # [None, 256]
h_drop_fc2 = tf.nn.dropout(h_relu_fc2, keep_prob=0.5)
# readout layer
readout = tf.matmul(h_drop_fc2, W_fc3) + b_fc3 # [None, 5]
